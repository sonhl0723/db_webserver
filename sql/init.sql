...
(
    ...
    `STATE`  ENUM('AVAILABLE')    NOT NULL    DEFAULT 'AVAILABLE' COMMENT '객실상태', -- enum값 설정
    PRIMARY KEY (NUM)
);
...
(
    ...
    `GENDER`          ENUM           NOT NULL    COMMENT '성', 
...
(
    `TYPE`              ENUM    NOT NULL    COMMENT '객실타입', 
    `NUM`               INT     NOT NULL    COMMENT '객실번호', 
...
ALTER TABLE ROOM_TYPE
    ADD CONSTRAINT FK_ROOM_TYPE_NUM_ROOM_NUM FOREIGN KEY (NUM)
        REFERENCES ROOM (NUM) ON DELETE RESTRICT ON UPDATE RESTRICT;


-- ADDRESS Table Create SQL
CREATE TABLE DEPARTMENT
(
    `ID`           ENUM    NOT NULL    COMMENT '부서ID', 
...
(
    ...
    `ROOM_TYPE`      ENUM        NOT NULL    COMMENT '객실타입', 
...
ALTER TABLE RESERVATION
    ADD CONSTRAINT FK_RESERVATION_ROOM_TYPE_ROOM_TYPE_TYPE FOREIGN KEY (ROOM_TYPE)
        REFERENCES ROOM_TYPE (TYPE) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
(
    ...
    `EP_STATE`      ENUM           NOT NULL    COMMENT '직원상태', 
    `LOGIN_ID`      VARCHAR(45)    NOT NULL    COMMENT '로그인아이디', 
    `LOGIN_PW`      VARCHAR(45)    NOT NULL    COMMENT '로그인비밀번호', 
    ...
    `DEP_ID`        ENUM           NOT NULL    COMMENT '부서ID', 
    `PERSON_ID`     INT            NOT NULL    COMMENT '사람_ID', 
    `RANK`          ENUM           NOT NULL    COMMENT '직급', 
    PRIMARY KEY (ID)
);
...
(
    ...
    `ROOM_TYPE`  VARCHAR(45)    NOT NULL    COMMENT '객실타입', 
    `ITEM`       ENUM           NOT NULL    COMMENT '비품', 
    `COUNT`      INT            NOT NULL    COMMENT '수량', 
    PRIMARY KEY (ID)
);

ALTER TABLE ROOM_AMENITY COMMENT '객실에 들어가는 기본적인 비품의 개수를 나타내는 테이블';
...
(
    ...
    `TYPE`         ENUM        NOT NULL    COMMENT '민원타입', 
    `START_TIME`   DATETIME    NOT NULL    COMMENT '민원발생시간', 
    `FIN_TIME`     DATETIME    NULL        COMMENT '민원종료시간', 
    `PRIORITY`     ENUM        NOT NULL    DEFAULT ''BASIC'' COMMENT '우선순위', 
    PRIMARY KEY (ID)
);
...
(
    ...
    `ATTENDANCE`  ENUM    NOT NULL    COMMENT '출근여부', 
    PRIMARY KEY (ID)
);
...
(
    ...
    `IS_COMPLETE`  ENUM           NULL        COMMENT '답변여부', 
...
ALTER TABLE RES_FAC
    ADD CONSTRAINT FK_RES_FAC_PACKAGR_ID_FACILITY_PACKAGE_PACKAGE_ID FOREIGN KEY (PACKAGR_ID)
        REFERENCES FACILITY_PACKAGE (PACKAGE_ID) ON DELETE RESTRICT ON UPDATE RESTRICT;
